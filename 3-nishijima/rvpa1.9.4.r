...
)
{
  ...
  if (TMB.compile) {
    library(TMB)
    compile("rvpa_tmb3.cpp")
    dyn.load(dynlib("rvpa_tmb3"))
  }
  ...
  if(!is.null(rec.new.index)) index0 <- index; index[rec.new.index, ncol(index)] <- NA
  ...
  if (kappa > 0) {
    if (is.null(b.fix)) b.init <- rep(1, nindex) else {
      b.init <- b.fix
      b.init[is.na(b.init)] <- 1
    }
    p.init0 <- p.init
    p.init <- c(p.init0, b.init)
  }
...
# core function for optimization
  ...
  p.est <- function(log.p, out=FALSE){
    ...
    if (!isTRUE(intercept)) {
      if (kappa > 0){
        p <- exp(log.p[-((length(log.p)):(length(log.p)-nindex+1))]) ###
      } else p <- exp(log.p)   # intercept==FALSEなら，パラメータはexp(log.p)
    }
  ...
  # tuning
   ...
   if (tune){
     if (est.constraint | !is.null(fixed.index.var)){
       ...
       Abund <- nn <- sigma <- b <- cv.abund <- NULL
       ...
       if (est.method=="ml"){
         ...
         for (j in 1:length(unique.sigma.constraint)) {
           rss <- NULL
           index.num <- which(sigma.constraint==unique.sigma.constraint[j])
           for (i in index.num) {
             abundance <- abund.extractor(abund=abund[i], naa, faa, dat, min.age=min.age[i], max.age=max.age[i], link=link[i], base=base[i], af=af[i], catch.prop=catch.prop, sel.def=sel.def, scale=scale)
             Abund <- rbind(Abund, abundance)
             if (!isTRUE(intercept)){
             ...
             rss <- c(rss,sum((log(as.numeric(index[i,avail]))-log(q[i])-b[i]*log(as.numeric(abundance[avail])))^2))
           }
           sigma[index.num] <- sqrt(sum(rss)/sum(nn[index.num]))
         }
         for (i in 1:nindex) {
           abundance <- abund.extractor(abund=abund[i], naa, faa, dat, min.age=min.age[i], max.age=max.age[i], link=link[i], base=base[i], af=af[i], catch.prop=catch.prop, sel.def=sel.def, scale=scale)
           avail <- which(!is.na(as.numeric(index[i,])))
      ...
      cv.abund <- c(cv.abund,sqrt(sum((log(as.numeric(abundance[avail]))-mean(log(as.numeric(abundance[avail]))))^2)/nn[i]))

       if (n.add==1 & !is.na(mean(index[,ny+n.add],na.rm=TRUE))){
         
         if(!is.null(rec.new.index)) {
           # if(est.method == "ls") new.naa[1] <- mean((index0[rec.new.index, ncol(index0)] / q[rec.new.index])^(1/b[rec.new.index]))*exp(-sigma^2/2)
           if(est.method == "ls") new.naa[1] <- exp(mean(log((index0[rec.new.index, ncol(index0)] / q[rec.new.index])^(1/b[rec.new.index]))))
           # if(est.method == "ml") new.naa[1] <- sum((sigma[rec.new.index])^(-2) * ((index0[rec.new.index, ncol(index0)] / q[rec.new.index])^(1/b[rec.new.index]))*exp(-sigma[rec.new.index]^2/2) / sum((sigma[rec.new.index])^(-2)))
           if(est.method == "ml") new.naa[1] <- exp(sum((sigma[rec.new.index])^(-2) * log((index0[rec.new.index, ncol(index0)] / q[rec.new.index]))*(1/b[rec.new.index])) / sum((sigma[rec.new.index])^(-2)))
      ...
      # if (penalty=="p") obj <- (1-kappa)*(1-lambda)*obj + lambda*sum(p^beta) + kappa*sum(abs(log(b)))     
      # 
      # if (penalty=="s") obj <- (1-kappa)*(1-lambda)*obj + lambda*sum((abs(saa[,ny]-apply(saa[, years %in% tf.year],1,get(stat.tf))))^beta) + kappa*sum(abs(log(b)))
      
      if (est.method == "ls") {
        if (penalty=="p") obj <- obj + lambda*((1-kappa)*sum(p^beta) + kappa*sum(pen.w*abs(b-1)/cv.abund))
        if (penalty=="s") obj <- obj + lambda*((1-kappa)*sum((abs(saa[,ny]-apply(saa[, years %in% tf.year],1,get(stat.tf))))^beta) + kappa*sum(pen.w*abs(b-1)/cv.abund))
      }
...
#  } else {
  if (isTRUE(TMB)){
    ...
    if (length(pen.w)==1) pen_w <- rep(1,nindex) else pen_w <- pen.w
    ...
    data2 <- list(Est=ifelse(est.method=="ls",0,1),b_fix=as.numeric(b_fix),alpha=alpha,lambda=lambda,beta=beta,Ab_type=Ab_type,Ab_type_age=Ab_type_age,w=index.w,af=af,CATCH=t(as.matrix(dat$caa)),WEI=t(as.matrix(dat$waa)),MAT=t(MAA),M=t(as.matrix(dat$M)),CPUE=t(index2),MISS=t(ifelse(index2==0,1,0)),Last_Catch_Zero=ifelse(isTRUE(last.catch.zero),1,0),kappa=kappa,pen_w=pen_w,sigma_constraint=sigma_constraint)
    
    if (kappa == 0) parameters <- list(log_F=log(p.init), log_b=rep(0,nindex)) else {
      parameters <- list(log_F=log(p.init0), log_b=log(b.init))
    }
    
    if (kappa > 0) map <- NULL else map <- list(log_b = factor(rep(NA,nindex)))
    
    obj <- MakeADFun(data2, parameters, map = map, DLL="rvpa_tmb4")
    if (optimizer == "nlm") {
      ...
      # if (kappa > 0) log.p.hat <- opt$estimate else {
      #   log.p.hat <- opt$estimate[-(length(opt$estimate)-nindex+1):(-length(opt$estimate))]
      # }
    }
    if (optimizer == "nlminb") {
      opt <- nlminb(obj$par, obj$fn, obj$gr, lower=obj$par*0+Lower, upper=obj$par*0+Upper)
      summary.p.est <- list()
      summary.p.est$estimate <- exp(opt$par)
      summary.p.est$minimum <- opt$objective
      summary.p.est$gradient <- NULL
      summary.p.est$code <- opt$convergence
      log.p.hat <- opt$par
      # if (kappa > 0) log.p.hat <- opt$par else {
      #   log.p.hat <- opt$par[-(length(opt$par)-nindex+1):(-length(opt$par))]
      # }
    }
  ...
  if (kappa > 0){
    term.f <- exp(log.p.hat[1:(length(log.p.hat)-nindex)])
    b <- exp(log.p.hat[(length(log.p.hat)-nindex+1):length(log.p.hat)])
  } else term.f <- exp(log.p.hat)
  ...
  res <- list(input=arglist, term.f=term.f, np=np, minimum=out$minimum, minimum.c=out$minimum.c, logLik=logLik, gradient=gradient, code=code, q=q, b=b, sigma=sigma, convergence=convergence, message=message, hessian=hessian, Ft=Ft, Fc.at.age=Fc.at.age, Fc.mean=Fc.mean, Fc.max=Fc.max, last.year=last.year, Pope=Pope, ssb.coef=ssb.coef, pred.index=pred.index, 
              # wcaa=caa*waa.catch,
              naa=naa, faa=faa, baa=baa, ssb=ssb, saa=saa)
...
SR.est.old <- function(res,model="BH",k=1,p.init=NULL,lower.limit=-25,scale=1,main=NULL,log=FALSE){
  ...
  Res <- list(SSB=SSB1,R=R1, k=k, p=p, res=res)

  return(Res)
}

SR.est.old2 <- function(res,model="BH",k=1,p.init=NULL,lower.limit=-25,scale=1,main=NULL,log=TRUE){
  SSB <- colSums(res$ssb,na.rm=TRUE)/scale
  R <- res$naa[1,]/scale
  ...
  if (isTRUE(log)){
    if (model=="BH") res <- nls(log(R1)~log.a+log(SSB1)-log(1+exp(log.b)*SSB1), data, start=list(log.a=p0[1],log.b=p0[2]), control=list(warnOnly=TRUE))
    if (model=="RI") res <- nls(log(R1)~log.a+log(SSB1)-exp(log.b)*SSB1, data, start=list(log.a=p0[1],log.b=p0[2]), control=list(warnOnly=TRUE))
  }
  else{
    if (model=="BH") res <- nls(R1~exp(log.a)*SSB1/(1+exp(log.b)*SSB1), data, start=list(log.a=p0[1],log.b=p0[2]), control=list(warnOnly=TRUE))
    if (model=="RI") res <- nls(R1~exp(log.a)*SSB1*exp(-exp(log.b)*SSB1), data, start=list(log.a=p0[1],log.b=p0[2]), control=list(warnOnly=TRUE))
  }
...
# retrospective analysis function for the Pacific chub mackerel
retro.est3 <- function(res, n=5, stat="mean", init.est=FALSE, b.fix=TRUE){
  res.c <- res
  res.c$input$plot <- FALSE
  Res <- list()
  obj.n <- obj.b <- obj.s <- obj.r <- obj.f <- NULL
  ...
  for (i in 1:n){
    ...
    obj.s <- c(obj.s, (sum(res1$ssb[,Y])-sum(res$ssb[,Y]))/sum(res$ssb[,Y]))
    obj.r <- c(obj.r, (res1$naa[1,Y]-res$naa[1,Y])/res$naa[1,Y])
    obj.f <- c(obj.f, (sum(res1$faa[-max.a,Y-1])-sum(res$faa[-max.a,Y-1]))/sum(res$faa[-max.a,Y-1])) #Fは一年前
  }
