...
Type objective_function<Type>::operator() ()
{
  ...
  if (Est==0){
    for (int k=0;k<K;k++){
      // f += w(k)*sigma2(k)+lambda*kappa*abs(log_b(k))/sd_log_abund(k);
      f += w(k)*sigma2(k)+lambda*kappa*abs(exp(log_b(k))-Type(1.0))*pen_w(k)/sd_log_abund(k);
    }
    f += lambda*(1-kappa)*sum(pow(exp(log_F),beta));
  }
  if (Est==1){
    for (int k=0;k<K;k++){
      for (int j=0;j<K;j++) {
    ...
    for (int k=0;k<K;k++){
      ...
      // f += lambda*kappa*abs(log_b(k))/sd_log_abund(k);
      f += lambda*kappa*abs(exp(log_b(k))-Type(1.0))*pen_w(k)/sd_log_abund(k);
    }
    f += lambda*(1-kappa)*sum(pow(exp(log_F),beta));
  }
  
  // f = f + lambda*((1-kappa)*sum(pow(exp(log_F),beta)) + kappa*sum(abs(log_b)));
  // 
  return f;
}
